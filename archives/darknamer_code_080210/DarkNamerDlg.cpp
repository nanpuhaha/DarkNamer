...
//	m_list.SetImageList(&m_img, LVSIL_SMALL);
	...
	m_list.InsertColumn(COL_ROOTPATH,_T("파일위치"));
...
void CDarkNamerDlg::OnDropFiles(HDROP hDropInfo) 
{
	...
	SetDlgItemText(IDC_ST_BAR, _T("처리중..."));
	for (i=0; i<cFiles; i++)
	{
...
{
	CDlgInput dlg;
	dlg.InitInputDlg(_T("이름의 앞에 지정한 문자열을 붙여줍니다."), _T("붙일 문자열"), _T(""));
...
{
	CDlgInput dlg;
	dlg.InitInputDlg(_T("이름의 뒤에 지정한 문자열을 붙여줍니다."), _T("붙일 문자열"), _T(""));
...
void CDarkNamerDlg::NameAddPath()
{
	...
	for (int i=0; i<m_list.GetItemCount(); i++)
	{
		...
		if (strPath!=strTemp) //같을때는 c:, d: 같은 드라이브 명일때.
		{
...
void CDarkNamerDlg::NameAddPathBack()
{
	...
	{
		CString strPath = m_list.GetItemText(i, COL_ROOTPATH);
		CString strTemp = strPath.Right(strPath.GetLength()-strPath.ReverseFind(_T('\\'))-1); //경로 맨뒤 자르기
		if (strPath!=strTemp) //같을때는 c:, d: 같은 드라이브 명일때.
		{
...
void CDarkNamerDlg::NameDigit()
{
	...
	dlg.m_aCombo.Add(_T("제일 뒷번호 맞춤"));
	dlg.m_aCombo.Add(_T("제일 앞번호 맞춤"));
...
void CDarkNamerDlg::NameAddNum()
{
	...
	dlg.m_aCombo.Add(_T("이름뒤에 번호붙임"));
	dlg.m_aCombo.Add(_T("이름앞에 번호붙임"));
	dlg.m_aCombo.Add(_T("폴더별로 뒤 번호붙임"));
	dlg.m_aCombo.Add(_T("폴더별로 앞 번호붙임"));
...
void CDarkNamerDlg::NameSamePath()
{
	...
	for (int i=0; i<m_list.GetItemCount(); i++)
	{
		//COL_ROOTPATH을 변경해 주면 나중에 실제 변경때 이값이 COL_NEWNAME과 결합되면서 새 경로가 된다
		m_list.SetItemText(i, COL_ROOTPATH, strPath); 
	}
	m_list.SetRedraw(TRUE);
}
...
void CDarkNamerDlg::NameDelPos()
{
	...
	dlg.m_aCombo.Add(_T("앞에서부터 삭제"));
	dlg.m_aCombo.Add(_T("제일 뒤부터 삭제"));
	if (dlg.DoModal()==IDCANCEL) return;
...
void CDarkNamerDlg::SortList()
{
	...
	dlg.m_aCombo.Add(_T("파일 이름에 따라 오름차순"));
	dlg.m_aCombo.Add(_T("파일 이름에 따라 내림차순"));
	dlg.m_aCombo.Add(_T("전체경로에 따라 오름차순"));
	dlg.m_aCombo.Add(_T("전체경로에 따라 내림차순"));
	dlg.m_aCombo.Add(_T("파일 크기에 따라 오름차순"));
	dlg.m_aCombo.Add(_T("파일 크기에 따라 내림차순"));
	dlg.m_aCombo.Add(_T("수정한 시각에 따라 오름차순"));
	dlg.m_aCombo.Add(_T("수정한 시각에 따라 내림차순"));
	dlg.m_aCombo.Add(_T("만든 시각에 따라 오름차순"));
	dlg.m_aCombo.Add(_T("만든 시각에 따라 내림차순"));
...
{
	//lParam1, lParam2는 비교될 두 리스트 항목의 ItemData값
	//주로 해당 항목의 인덱스 값을 그대로 넣어서 쓴다
	//lParamSort는 SortItems의 두번째 인자값(주로 해당 리스트 포인터)
